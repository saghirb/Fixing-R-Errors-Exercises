...
ctwide <-  ct %>% select(Continent, Year, Total) %>%
...
ctwideB <-  ct %>%
  mutate(TotalB = paste(round(Total/1e9, 2),"B")) %>%
...
ctprop %>%   ggplot(aes(Year, PropF, colour=Continent)) +
  ...
  scale_y_continuous(limits = c(.45, .55), breaks = seq(0.45, 0.55, 0.01)) +
