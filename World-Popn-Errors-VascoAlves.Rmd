...
```{r freadData}
# the packages have wrong names
# it's missing a parentheses on wpOrig
library(dplyr)
library(tidyr)
library(readr)
library(here)
...
```{r preprocess}
# the name was incorrect wp0rig --- wpOrig
wp <- wpOrig %>% 
...
# it's missing a ".," on ggplot function
...
# the name was incorrect ctt --- ct
ctwide <-  ct %>% select(Continent, Year, Total) %>% spread(Continent, Total) 
...
#it's missing parantheses on mutate command
ctwideB <-  ct %>% 
...
```{r ContinentProp}
# the names were incorrect: PopF --- TotF, PopM --- TotM
ctprop <- ct %>% mutate(PropF = TotF/Total, PropM = TotM/Total)
...
```{r ContinentPropPlot}
library(ggplot2)
ctprop %>% ggplot(aes(x=Year, y=PropF, colour=Continent)) + geom_line() +
...
ctwidePct <-  ctprop %>% 
  mutate(PctF = paste(format(round(100*PropF, 1), trim = TRUE), "%")) %>%
