...
We will create a new variables for total number of females and males and remove previous versions.

```{r preprocess}
wp <- wpOrig %>% 
  rename(Continent = continent, Country = Location, Year = Time) %>% 
  mutate(PopF = 1000*PopFemale, 
         PopM = 1000*PopMale,
         PopT = PopF + PopM) %>% 
...
```{r ContinentPropPlot}
library(ggplot2)
ctprop %>%   ggplot(aes(Year, PropF), colour=Continent) +
  geom_line() +
