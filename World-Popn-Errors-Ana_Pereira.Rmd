...
wp <- wpOrig %>% 
  ...
  mutate(PopF = 1000*PopFemale,PopM = 1000*PopMale,PopT = PopF + PopM) %>% 
...
library(ggplot2)
ctprop %>%   
  ggplot(aes(Year, PropF), colour=Continent) + geom_line() +  geom_hline(yintercept = 0.5, lty=3) + scale_y_continuous(limits = c(.45, .55), breaks = seq(0.45, .55, 0.01)) +  ylab("Female Proportion") 
```
